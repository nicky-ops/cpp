...
const char* stdName[numStandards] = {"Pre-C++11", "C++11", "C++14", "C++20", "C++23", "C++26"};

long getCPPStandard()
{
    // Visual Studio is non-conforming in support for __cplusplus (unlsess you set a specific compiler flag, which you probably haven't)
    // In Visual Studio 2015 or newer we can use _MSVC_LANG instead
    // See https://devlogs.microsoft.com/cppblog/msvc-now-correctly-reports-__cplusplus/
...
int main()
{
    ...
    for (int i = 0; i < numStandards; i++)
    {
        ...
        // If the reported version is between two finalized standard codes, this must be a preview / experimental support for the next upcoming version.
        if (standard < stdCode[i])
        {
            std::cout << "Your compiler is using a preview/pre-release of " << stdName[i]
                <<" (language standards code " << standard << "L)\n";
                break;
        }
    }
